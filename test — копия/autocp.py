...
class AutoCP:
    def __init__(self):
        ...
        # Убедитесь, что ширина (135) точно соответствует 100% полосы
    ...
    def get_cp_percentage(self, frame):
        """
        Вычисляет процент заполненности CP-полосы, основываясь на ее длине.
        Это дает результат 1 к 1.
        """
        ...
        # Находим координаты всех желтых пикселей
        coords = cv2.findNonZero(mask)
        
        if coords is None:
            return 0  # Если желтых пикселей нет, CP равен 0%
            
        # Находим самую правую точку (максимальную координату X)
        max_x = np.max(coords[:, :, 0])
        
        # Полная ширина полосы (из константы cp_bar_area)
        full_width = self.cp_bar_area[2]
        
        # Вычисляем процент от полной ширины
